...
class Vault {
  ...
  public async start({ key }: { key: Buffer}) {
    ...
    this.logger.info(
      `Created vault directory at ${this.baseDir}`,
    );
  ...
  public async stop() {
    ...
    try {
      ...
      this.logger.info(
        `Destroyed vault directory at ${this.baseDir}`,
      );
    } finally {
      release();
    }
  ...
  public async addSecret(
    ...
    try {
      ...
      this.logger.info(
        `Wrote secret to directory at '${secretName}'`,
      );
  ...
  public async updateSecret(
    ...
    try {
      ...
      await writeFile(secretName, content, {});
      this.logger.info(
        `Updated secret at directory '${secretName}'`,
      );
  ...
  public async renameSecret(
    ...
    try {
      ...
      if (!await exists(currSecretName)) {
        throw new errors.ErrorSecretUndefined(`${currSecretName} does not exist`);
      }
      ...
      if (
        (await exists(currSecretName)) ||
        !(await exists(newSecretName))
      ) {
        return false;
      }
      return true;
    ...
    try {
      ...
      if ((await stat(secretName)).isDirectory()) {
        if (recursive) {
          ...
          this.logger.info(
            `Deleted directory at '${secretName}}'`,
          );
          await this.commitChanges(
            [
              {
      ...
      for await (const secretPath of vaultsUtils.readdirRecursively(
        ...
        // If existing path exists, write secret
          ...
          try {
            ...
            this.logger.info(
              `Updated secret at directory '${relPath}'`,
            );
            commitList.push({
              fileName: relPath,
              action: 'modified'
          ...
          try {
            ...
            await writeFile(relPath, content, {});
            this.logger.info(
              `Wrote secret to directory at '${relPath}'`,
            );
            commitList.push(
              {
                fileName: relPath,
  ...
  /**
   * Changes the branch of the vault if
   *
   * @param fileChanges changes that have been made to files
   * @param message description of change
   * @returns true if fast-forward possible, false otherwise
   */
  private async changeBranch(): Promise<void> {
    if (
      (await git.getConfig({
        fs: this.efs,
        dir: '',
